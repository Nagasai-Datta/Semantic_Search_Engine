...
All commands must be run from the project root (`semantic-search/`), never from inside `src/` or `analysis/`.
...
The two queries are phrased differently but mean the same thing. The cache recognises this via cosine similarity on the embedding vectors and returns the stored result without recomputation.
...
category: talk.politics.guns   entropy: 2.41
  ...
  "The Senate bill on gun control passed yesterday..."
```

A hard clustering model would force this into a single incorrect label. The GMM correctly represents the ambiguity.
...
The FastAPI service can be containerised for consistent deployment. The data files must be generated locally first (steps 1–4 above) before building the image, as they are baked into the container.
...
The API is available at **http://localhost:8000/docs** in both cases.

**How it works:**

The `Dockerfile` uses a `python:3.11-slim` base image and copies the pre-built data files (`chroma_db/`, `cluster_memberships.npy`, `cluster_centers.npy`, `pca_model.joblib`) directly into the image at build time. This means the container is fully self-contained — no external database, no volume mounts, no internet required at runtime.
...
These files are generated by running the pipeline and are excluded from version control. They can be fully regenerated by re-running the steps above.
